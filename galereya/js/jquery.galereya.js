    ...
    function Galereya(options) {
        ...
        var buildDOM = function () {
            if (categories.length > 0) {
                $categoriesList = $('.dropdown-menu');
                //$header = $('<h1>Wildphoto</h1> <p>Прямиком с лугов Полесья</p>');
                //$top = $('<div class="galereya-top" />');
                //self.prepend($top.html($categoriesList));
                //$top.append($header);
                $categoriesList.append('<a class="dropdown-item">' + self.options.noCategoryName + '</a>');
                for (var i = 0; i < categories.length; i++) {
                    //$categoriesList.append('<li class="galereya-cats-item"><span>' + categories[i] + '</span></li>');
                    $categoriesList.append('<a class="dropdown-item">' + categories[i] + '</a>');
                }
            }
        ...
        var loadImages = function (index, category) {
            ...
            if (category && !data[index].category.includes(category)) {
                setTimeout(function () {
        ...
        var changeCategory = function (category) {
            ...
            $categoriesList.empty().prepend('<a class="dropdown-item">' + category + '</a>');
            ...
            for (var i = 0, len = categories.length, cat; i < len; i++) {
                cat = categories[i];
                if (cat !== category) {
                    $categoriesList.append('<a class="dropdown-item">' + cat + '</a>');
                }
            }
        };
